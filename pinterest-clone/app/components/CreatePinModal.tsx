...
interface CreatePinFullScreenProps {
  onPinCreated: () => void
  onClose?: () => void
}

export default function CreatePinFullScreen({ onPinCreated, onClose }: CreatePinFullScreenProps) {
  ...
  return (
    <main className="min-h-screen bg-gray-50 p-4 flex flex-col">
      ...
      <form
        ...
        className="flex flex-col flex-grow max-w-4xl mx-auto w-full bg-white rounded-lg shadow-md p-6 overflow-y-auto"
        style={{ maxHeight: 'calc(100vh - 96px)' }} // Ajusta altura para compensar padding y header
      >
          ...
          <div className="flex flex-col flex-grow overflow-auto">
            ...
            {preview && (
              <img
                src={preview}
                alt="Vista previa"
                className="max-h-40 w-auto rounded-lg mt-4 shadow mx-auto"
        ...
        <div className="flex justify-end space-x-4 pt-6 flex-shrink-0">
          {onClose && (
            <button
              type="button"
              onClick={onClose}
              className="px-5 py-2 text-gray-600 hover:text-gray-900 bg-gray-100 rounded"
              disabled={loading}
            >
              Cancelar
            </button>
